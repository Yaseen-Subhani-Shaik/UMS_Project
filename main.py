...
if menu_choice == "Create College":
    ...
    if not st.session_state.colleges:
        ...
        clgname = st.selectbox("Choose college", [c.cname for c in st.session_state.colleges])
        subject = st.text_input("Enter Your Subject") # Fixed to text_input for course names
        tname = st.text_input("Enter teacher name") # Fixed placeholder text from student to teacher
        branch = st.text_input("Enter your branch")
        if st.button("ADD TEACHER"):
            if not (subject and tname and clgname and branch):
                ...
                clg_ob = find_college(clgname)  # find the college object based upon college name
                teacher_obj = teacher(subject, tname, branch)  # created teacher object 
                clg_ob.add_teacher(teacher_obj) # Fixed method call from add_student to add_teacher
    ...
    if not st.session_state.colleges:
        ...
        clgname = st.selectbox("Choose College", [c.cname for c in st.session_state.colleges]) # Fixed property from c.name to c.cname
        clg_ob = find_college(clgname)
        st.subheader(f"List of students : {clgname}")
        if clg_ob.students:
            for i, s in enumerate(clg_ob.students, 1): # Fixed stray double quote syntax error
                st.write(i, " : ", s.name) # Fixed variable scope from sname to s.name
        else:
    ...
    if not st.session_state.colleges:
        ...
        if clg_ob.teachers:
            for i, t in enumerate(clg_ob.teachers, 1):
                st.write(i, " : ", t.name, " (", t.subject, ")")
        else:
            st.warning("No teacher found")

elif menu_choice == "List of Colleges":
    if not st.session_state.colleges:
        st.info("No colleges created yet.")
    else:
        st.subheader("Registered Colleges")
